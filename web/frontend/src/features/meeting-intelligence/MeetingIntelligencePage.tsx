...
export function MeetingIntelligencePage() {
  ...
  return <Layout><div className="mi-page">
    <div className="mi-station-strip" aria-label="System status">
      <div className="mi-station-device"><strong>Meeting Station</strong><span className={`mi-status-dot ${connected ? 'is-online' : ''}`} /><span>{connected ? 'Online' : token ? 'Connecting…' : 'Not paired'}</span></div>
      <div className="mi-station-device"><strong>Local AI Engine</strong><span className={`mi-status-dot ${connected && caps?.station.worker_connected ? 'is-online' : ''}`} /><span>{connected && caps?.station.worker_connected ? 'Online' : 'Offline'}</span></div>
    ...
    {connected && !caps?.station.worker_connected && <div className="mi-notice"><div><strong>Inference engine offline.</strong> Recordings remain available and processing resumes when the inference worker reconnects.</div></div>}
    {connected && missingSpeechModels && <div className="mi-notice mi-warning"><AlertCircle /><div><strong>A local speech model is missing.</strong> A selected transcription model is not available. Recordings can still be archived and retried after the local model setup is complete.</div></div>}
...
function UploadDialog({ open, onOpenChange, config, maxBytes, diarizationAvailable, onCreated }: { open: boolean; onOpenChange: (value: boolean) => void; config: WorkerConfig; maxBytes?: number; diarizationAvailable?: boolean; onCreated: (job: JobRecord) => void }) {
  ...
  function chooseFile(value?: File) {
    ...
    if (value && !/\.(mp3|wav|m4a|webm|caf|ogg|flac|mp4|mkv)$/i.test(value.name)) { setError('Choose an MP3, WAV, M4A, WebM, CAF, OGG, FLAC, MP4 or MKV recording.'); return }
  ...
  return <Dialog open={open} onOpenChange={value => { if (!busy) onOpenChange(value) }}><DialogContent className="mi-dialog" showCloseButton={!busy}><DialogHeader><DialogTitle>Import meeting</DialogTitle><DialogDescription>Import an audio or video recording, or process an existing transcript. Participants must have been informed about recording and local AI transcription.</DialogDescription></DialogHeader><form className="mi-form" onSubmit={event => void submit(event)}><fieldset disabled={busy}>
    ...
    {mode === 'audio' && <label className="mi-checkbox"><input type="checkbox" checked={diarization && diarizationAvailable !== false} disabled={diarizationAvailable === false} onChange={event => { setDiarization(event.target.checked); submission.current = '' }} /><span>Separate speakers<small>{diarizationAvailable === false ? 'Speaker separation is not available on the inference worker.' : 'Uses the diarization models on the local inference worker.'}</small></span></label>}
    </fieldset>
...
function RecordDialog({ open, onOpenChange, config, diarizationAvailable, onCreated }: { open: boolean; onOpenChange: (value: boolean) => void; config: WorkerConfig; diarizationAvailable?: boolean; onCreated: (job: JobRecord) => void }) {
  ...
  return <Dialog open={open} onOpenChange={value => { if (!busy) onOpenChange(value) }}><DialogContent className="mi-dialog" showCloseButton={!busy}><DialogHeader><DialogTitle>Record on station</DialogTitle><DialogDescription>Use the station microphone. Inform participants about recording and local AI transcription before starting.</DialogDescription></DialogHeader><form className="mi-form" onSubmit={event => void record(event)}><fieldset disabled={busy}><label htmlFor="mi-record-title">Meeting title</label><Input id="mi-record-title" required maxLength={200} placeholder="e.g. Team check-in" value={meetingTitle} onChange={event => setMeetingTitle(event.target.value)} /><label>Audio language<select value={language} onChange={event => setLanguage(event.target.value)}><option value="kk_ru">Қазақша + русский (auto)</option><option value="kk">Қазақша</option><option value="ru">Русский</option><option value="en">English</option><option value="auto">Auto detect</option></select></label><label className="mi-checkbox"><input type="checkbox" checked={diarization && diarizationAvailable !== false} disabled={diarizationAvailable === false} onChange={event => setDiarization(event.target.checked)} /><span>Separate speakers<small>{diarizationAvailable === false ? 'Speaker separation is not available on the inference worker.' : 'Uses the diarization models on the local inference worker.'}</small></span></label><div className="mi-record-source"><strong>Station microphone</strong><span>Connected to Meeting Station</span></div></fieldset><ErrorText error={error} /><Button type="submit" disabled={busy || !meetingTitle.trim()}>{busy ? <Loader2 className="animate-spin" /> : <Mic />}Start recording</Button></form></DialogContent></Dialog>
}
...
function MeetingDetail({ job, config, connected, onError, onRetry, onCancel, busy }: { job: JobRecord; config: WorkerConfig; connected: boolean; onError: (value: string) => void; onRetry: () => void; onCancel: () => void; busy: boolean }) {
  ...
  return <div>
    <header className="mi-detail-header"><div className="mi-detail-topline"><span>{date(job.created_at, true)}</span><Badge stage={job.stage} /></div><h2>{title(job)}</h2><div className="mi-detail-meta"><span>{job.source_kind === 'text' ? 'Imported transcript' : 'Audio recording'}</span>{job.station?.source_bytes ? <span>{size(job.station.source_bytes)}</span> : null}{result?.protocol.metadata.duration_seconds != null && <span>{time(result.protocol.metadata.duration_seconds)}</span>}{result?.transcript.language && <span>{result.transcript.language.toUpperCase()}</span>}</div>{result && <div className="mi-exports"><select aria-label="Export format" value={format} onChange={event => setFormat(event.target.value as typeof format)}><option value="pdf">PDF report</option>{result.exports.docx && <option value="docx">Word document (.docx)</option>}<option value="csv">CSV action items</option><option value="json">JSON result</option><option value="ics">Calendar tasks (.ics)</option></select><Button variant="outline" size="sm" disabled={exporting || !connected} onClick={() => void exportFile()}>{exporting ? <Loader2 className="animate-spin" /> : <Download />}Export</Button><Button size="sm" variant="outline" disabled={!connected || reviewOpen} onClick={() => setReviewOpen(true)}>Review &amp; correct</Button></div>}</header>
    ...
    {processing && <div className="mi-processing"><div><Loader2 className="animate-spin" /><strong>{stageNames[job.stage]}</strong><Button variant="ghost" size="sm" disabled={busy || !connected} onClick={onCancel}>Cancel processing</Button></div><div className="mi-processing-track" aria-label={`Processing stage: ${stageNames[job.stage]}`}>{stages.filter(stage => stage !== 'diarizing' || job.manifest?.diarization).map(stage => <span key={stage} className={stages.indexOf(stage) <= stages.indexOf(job.stage) ? 'is-done' : ''} />)}</div><p>{job.stage === 'queued' ? 'Your source is saved. The Mac processes one meeting at a time.' : 'Working locally on the local inference worker. You can leave this page and return to the archive.'}</p></div>}
    ...
    {result && <>
      ...
      {Boolean(result.review_history?.length) && <details className="mi-review-history"><summary>Review history · revision {result.review_revision}</summary>{result.review_history?.slice().reverse().map(event => <article key={event.revision}><strong>{event.reviewer}</strong><span>{date(event.reviewed_at, true)} · {event.changes.length} change(s)</span>{event.note && <p>{event.note}</p>}</article>)}</details>}
