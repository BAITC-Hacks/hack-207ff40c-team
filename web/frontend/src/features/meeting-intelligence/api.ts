...
export interface MeetingResult {
  ...
  review_history?: Array<{ revision: number; reviewer: string; reviewed_at: string; note: string; changes: Array<{ kind: string; id: string; before: unknown; after: unknown }> }>
}
export interface ActionItem extends SourcedItem {
  task: string
  assignee?: string | null
  deadline_text?: string | null
  deadline_date?: string | null
  priority: 'low' | 'medium' | 'high' | 'urgent' | 'not_specified'
}
export interface ReviewRequest {
  ...
  note: string
  speaker_names: Record<string, string>
  actions: Array<Pick<ActionItem, 'id' | 'task' | 'assignee' | 'deadline_text' | 'deadline_date' | 'priority'> & { review_status: 'human_confirmed' | 'needs_review' | 'rejected' }>
  findings: Array<{ id: string; review_status: 'human_confirmed' | 'needs_review' | 'rejected' }>
  new_actions?: Array<Pick<ActionItem, 'task' | 'assignee' | 'deadline_text' | 'deadline_date' | 'priority'> & { segment_ids: string[]; review_status: 'human_confirmed' | 'needs_review' }>
}
...
export const saveReview = (config: WorkerConfig, id: string, review: ReviewRequest) => json<MeetingResult>(config, `/v1/jobs/${encodeURIComponent(id)}/review`, {
  method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(review), signal: AbortSignal.timeout(120000),
})
...
export async function downloadExport(config: WorkerConfig, id: string, format: 'json' | 'csv' | 'pdf' | 'docx' | 'ics'): Promise<void> {
