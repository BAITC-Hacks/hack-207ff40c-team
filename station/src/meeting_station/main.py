...
AUDIO_EXTENSIONS = {".mp3", ".wav", ".m4a", ".webm", ".ogg", ".caf", ".flac", ".mp4", ".mkv"}
...
def create_app(settings=None, mac=None, start_worker=True, browser=None):
    ...
    async def review(job_id: UUID, body: dict):
        identity = str(job_id)
        async with app.state.review_lock:
            app.state.store.result(identity)  # A completed local report is required.
            try:
                reviewed = await app.state.mac.json("POST", "/v1/jobs/{}/review".format(identity), json=body)
                await app.state.worker.archive_result(identity, reviewed)
            except WorkerUnavailable as exc:
                if exc.code == "ENGINE_HTTP_409":
                    # Keep Reload useful after another client saved first.
                    # Failure to refresh must preserve the old complete bundle.
                    with contextlib.suppress(WorkerUnavailable):
                        current = await app.state.mac.json("GET", "/v1/jobs/{}/result".format(identity))
                        await app.state.worker.archive_result(identity, current)
                    raise HTTPException(409, "The report changed. Reload it before saving your review") from exc
                if exc.code == "ENGINE_HTTP_422":
                    raise HTTPException(422, "Invalid review. Check the fields and selected finding") from exc
                raise HTTPException(503, "Review could not be synchronized. The previous local report remains available; retry the same save") from exc
            return app.state.store.result(identity)

    @app.get("/v1/jobs/{job_id}/export/{format_name}")
    async def export(job_id: UUID, format_name: str):
        if format_name not in {"pdf", "docx", "json", "csv", "ics"}:
            raise HTTPException(404, "Export format not found")
        report = app.state.store.result(str(job_id))
        directory = app.state.store.exports / str(job_id)
        if report.get("review_revision", 0):
            directory /= "review-{}".format(report["review_revision"])
        path = directory / ("meeting." + format_name)
        if not path.exists():
