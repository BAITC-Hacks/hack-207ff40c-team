...
class Store:
    ...
    def complete(self, job_id, result):
        with self.transaction() as db:
            ...
            if payload["stage"] == "cancelled":
                return
            if row["result"] and json.loads(row["result"]).get("review_revision", 0) > result.get("review_revision", 0):
                return  # A slow archive refresh must not overwrite a newer review.
            ...
            result["job"] = payload
            formats = ["json", "csv", "pdf", "ics"] + (["docx"] if "docx" in result.get("exports", {}) else [])
            result["exports"] = {format_name: "/v1/jobs/{}/export/{}".format(job_id, format_name) for format_name in formats}
