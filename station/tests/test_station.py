...
def test_archives_video_containers_before_local_audio_extraction(client, suffix):
    response = client.post('/v1/jobs', data={'manifest_json': json.dumps({'meeting_id': str(uuid4())})},
        files={'audio': ('meeting' + suffix, b'container-fixture', 'application/octet-stream')})
    assert response.status_code == 202
    assert client.get('/v1/jobs/' + response.json()['id'] + '/audio').content == b'container-fixture'
