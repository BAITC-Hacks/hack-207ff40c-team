...
def test_accepts_video_recordings_for_local_audio_extraction(tmp_path, suffix):
    with TestClient(create_app(settings(tmp_path), start_worker=False)) as client:
        response = client.post('/v1/jobs', headers=AUTH,
            data={'manifest_json': json.dumps(MANIFEST)}, files={'audio': ('meeting' + suffix, b'container-fixture', 'application/octet-stream')})
        assert response.status_code == 202
        assert response.json()['source_kind'] == 'audio'
        assert response.json()['source_path'].endswith(suffix)
