...
## Последнее изменение: интерфейс и снимки README

Интерфейс станции переведён на русский и обновлён без новых зависимостей:
чёткая навигация, приоритет поручений, читаемые источники, форма проверки,
светлая и тёмная темы, мобильная компоновка. Контракты API сохранены.
Четыре снимка в основном тексте README заменены свежими снимками приложения.
Использованы настоящие локальные сервисы и явно синтетические данные;
модели не запускались, внешних запросов при съёмке не было.

`python3 scripts/harness.py verify` завершился с кодом 0: все восемь проверок
пройдены — 34 теста harness, 237 тестов станции/вычислителя, 95 тестов адаптеров,
33 браузерные регрессии, два реальных сценария проверки и экспорта, сборка и lint.
[Отчёт и исходные журналы](verification/ui-refresh-20260923/verification.json).
После последних правок порядка разделов и мобильной компоновки сборка и
съёмка интерфейса повторно прошли успешно; версия проверки сохраняется после
перезагрузки, ошибок браузера нет. Проверены ширина 390 пикселей и тёмная тема.
[Запись съёмки](assets/readme/capture-receipt.json).

Изменены компоненты станции и их CSS в `web/frontend/src`, два браузерных
набора тестов, `scripts/check-ui.py`, четыре снимка и их реестр, этот handoff,
`EXPERIMENT.md` и `ATTRIBUTION.md`. Снимки уже подключены существующими ссылками
README. Следующий шаг: проверить представление README в удалённом репозитории
после отправки коммита. Аппаратные и модельные ограничения сохраняются.

## Предыдущая работа: русскоязычный README и схемы

По последующему уточнению пользователя раскрывающиеся блоки README убраны:
все скриншоты, изображение корпуса, GIF и аппаратные результаты показаны сразу
в основном тексте. Проверка разметки и `git diff --check` пройдены.
